...
def arguments():
    ...
    parser.add_argument("--formats", dest="supportedFormats",
    ...
    generalGroup.add_argument("--max-width", dest="maxWidth",
        type=int, default=0, help="Maximum width in pixels")
    
    generalGroup.add_argument("--max-height", dest="maxHeight",
        type=int, default=0, help="Maximum height in pixels")
    
    generalGroup.add_argument("--grayscale",
    ...
    generalGroup.add_argument("--no-comparison",
    ...
    jpegGroup.add_argument("--quality", dest="imageQuality", type=int,
        help="Specify a fixed quality setting for JPEG files (an integer value, between 1 and 100).")
    
    jpegGroup.add_argument("--keep-exif", dest="keepExif", action="store_true",
        help="Keep image EXIF data (this is discarded by defualt).")

    pngGroup = parser.add_argument_group("PNG specific options",
        description="These options apply only to PNG images")

    pngGroup.add_argument("--convert", dest="convertToJPEG", action="store_true",
        help="Convert PNG to JPEG. Keeps the original PNG image.")
    ...
    return sourcePath, destinationPath, args.imageQuality, args.maxWidth, args.maxHeight, \
            args.keepExif, args.convertToJPEG, args.grayScale, args.noComparison, displayConfig
